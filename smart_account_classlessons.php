...
//Create The counts for the left Menu
/* Execute the query. */     
$tsql = "select count(*) from lesson where schoolno =  '".$_COOKIE['schoolno']."' and classno = '".$_COOKIE['classno']."' and (select count(*) from class where lesson.classno = class.classno and 
...
$tsql = "Select lessonno,lesson,chapterno,standard,CONVERT(VARCHAR(10),startdate,110) as startdate1,CONVERT(VARCHAR(10),duedate,110) as duedate1,objective from lesson where SchoolNo  = '".$_COOKIE['schoolno']."' and classno = '".$_COOKIE['classno']."' And Active= 1 ORDER BY lessonno ";    
  

$stmt = sqlsrv_query( $conn, $tsql );


$data = " <table class='table table-striped table-bordered' style='font-size:11px'>
                        <thead>
                            <tr>
...
{	
	...
	$data  = $data . "<tr class='special' style='font-size:10px'><th style='width: 10%'><a href='javascript:OpenLessonEdit(".$row[0].");' >".$row[0]."</a></th>";
	$data  = $data . "<th style='width: 10%'>".$row[1]."</th>";
	$data  = $data . "<th style='width: 35%'>".$row[2]."</th>";
	$data  = $data . "<th style='width: 8%'>".$row[3]."</th>";
	$data  = $data . "<th style='width: 8%'>".$row[4]."</th>";
	$data  = $data . "<th style='width: 8%'>".$row[5]."</th>";
    $data  = $data . "<th style='width: 20%'>".$row[6]."<th></tr>";	
...
$smarty->display('smart_account_classlessons.tpl');
