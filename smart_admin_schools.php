...
$tsql = "Select SchoolNo,School,City from School where schoolno =  '".$SchoolNoLocal."' ORDER BY SCHOOL";     
  

//where SchoolNo = '".$SchoolNoLocal."' 
...
$data = "<table id='grid-selection' class='table table-condensed table-hover table-striped'>
    <thead>
        <tr>
            <th  data-column-id='schoolno' data-identifier='true'>School #</th>
             <th data-formatter='link' data-column-id='school'>School</th>
...
{	
	...
	$data  = $data . "<tr><td>".$row[0]."</td>";
	$data  = $data . "<td><a href='javascript:OpenSchoolView(".$row[1].");' >".$row[1]."</a></td>";
    $data  = $data . "<td>".$row[2]."</td></tr>";	
...
$smarty->display('smart_admin_schools.tpl');
